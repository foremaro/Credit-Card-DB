...
/****************************************************
 * The is a unit test for the Credit Card Database class
 *
 * @author Scott Grissom
 * @version Februrary 15, 2015
 **************************************************/
public class CreditCardTest
{
    ...
    public void testCustomerConstructor()
    {
        ...
        Assert.assertEquals("Customer() first name does not match", 
                "Joe", cust.getFirst());                
        Assert.assertEquals("Customer() last name does not match", 
                "Smith", cust.getLast());  
        Assert.assertEquals("Customer() credit card does not match", 
                "Discover", cust.getCreditCard());
        Assert.assertFalse("Customer() gender does not match", 
                cust.isFemale());                  
    ...
    public void testCardSummary()
    {
        ...
        Assert.assertTrue("getCardSummary() average not correct, did you format the results?", 
                str.contains("6,200"));                
        str = db.getCardSummary("MasterCard");
        Assert.assertTrue("getCardSummary() not correct", 
                str.contains("12,000"));   
        str = db.getCardSummary("Visa");
        Assert.assertTrue("getCardSummary() what if 'cards not found'?", 
